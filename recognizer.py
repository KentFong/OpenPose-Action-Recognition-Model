...
def framewise_recognize(pose, pretrained_model):
    ...
    if bboxes:
        ...
        for d in trk_result:
            ...
            # id = int(d[4])
            try:
                # x center starting from the neck
                # distance between track box and human x center
                tmp = np.array([abs(i - (xmax + xmin) / 2.) for i in xcenter])
                j = np.argmin(tmp)
            except:
                # if no humans, j=0
            ...
            if joints_norm_per_frame.size > 0:
                ...
                # pred = pretrained_model.predict_predict(joints_norm_single_person)
                if pretrained_model.__class__.__name__ in ['XGBRegressor', 'XGBClassifier']:
                    pred_prob = pretrained_model.predict_proba(joints_norm_single_person)
                    pred = np.argmax(pred_prob)
                elif pretrained_model.__class__.__name__ in ['MLPClassifier', 'QuadraticDiscriminantAnalysis']:
                    pred_prob = pretrained_model.predict_proba(joints_norm_single_person)
                    # print(pred_prob)
                    pred = np.argmax(pred_prob[0])
                else:
                    pred_prob = pretrained_model.predict(joints_norm_single_person)
                    pred = np.argmax(pred_prob)
                data = {'clapping':pred_prob[0][0],
                        'punching':pred_prob[0][1],
                        'hittingBottle':pred_prob[0][2],
                        'hittingStick':pred_prob[0][3],
                        'jogging_side':pred_prob[0][4],
                        'jogging_fb':pred_prob[0][5],
                        'kicking':pred_prob[0][6],
                        'running_side':pred_prob[0][7],
                        'running_fb':pred_prob[0][8],
                        'walking_side':pred_prob[0][9],
                        'walking_fb':pred_prob[0][10],
                        'wavingHands':pred_prob[0][11]
                                                                        }
                init_label = Actions(pred).name
